...
async function generate(currentImages, cachedImages, platformsDir, platform, projectName) {
    ...
    await Promise.all(currentImages.map((currentImage) => {
        ...
        if (
            ...
            return createImage(currentImage, resizedImages, platformsDir, platform, projectName);
        } else {
            return Promise.resolve();
        }
    }));
...
function getResizedImages({ basename }, platform, projectName) {
    const relativePath = getPlatformRelativeResourcesDirectoryPath(platform, projectName);
...
async function createImage({ filepath, basename, scale }, resizedImages, platformsDir, platform, projectName) {
    if (platform === 'ios') {
        const platformResourcesDirectoryPath = getPlatformResourcesDirectoryPath(platformsDir, platform, projectName);
        const folderPath = path.join(platformResourcesDirectoryPath, `${basename}.imageset`);
...
function getPlatformRelativeResourcesDirectoryPath(platform, projectName) {
    if (platform === 'ios') {
        return path.join(projectName, 'Resources/Assets.xcassets');
    } else if (platform === 'android') {
...
function getPlatformResourcesDirectoryPath(platformsDir, platform, projectName) {
    return path.join(platformsDir, platform, getPlatformRelativeResourcesDirectoryPath(platform, projectName));
}
