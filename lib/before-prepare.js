...
async function getImagesPathsList(path) {
    try {
        ...
        return items
            ...
            .map(file => path.join(path, file.name))
            .sort();
    } catch (error) {
...
async function createImage(filepath, basename, platformResourcesDirectoryPath, platform) {
    ...
    if (platform === 'ios') {
        ...
        await writeFile(path.join(folderPath, 'Content.json'), getIOSDataFileContent(basename));
